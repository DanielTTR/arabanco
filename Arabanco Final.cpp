...
class Banco {
	...
	public:
		...
		void VerifContas (int Conta, int Ag, bool &t, int &cnt) {
			int c, a;
			// verificando se a conta realmente existe no banco
			for (int i = 0; i < id; i++) {
				c = V[i].getnumConta();
				a = V[i].getnumAgencia();
				if (c == Conta && a == Ag) {
        ...
        void ExtratoNaTela(int &cnt){
            ...
            cout << "\nNOME: " << V[cnt].getnomeTitular() << "\t" << "Conta/Agencia: " << V[cnt].getnumConta() << "/" << V[cnt].getnumAgencia() << "\n";
            cout << "Data: " << tm.tm_mday <<"/"<< tm.tm_mon + 1 <<"/"<< tm.tm_year + 1900 << "\t" << "Hora: " << time << "\n";

            for(int i = 0; i < V[cnt].getNummovimentacao(); i++){
                cout <<V[cnt].getmovimentacao(i).gethistorico() <<" "<< V[cnt].getmovimentacao(i).getvalor()<<endl;
            }
            ...
            case 9:
                ...
                switch(resp_extrato){
                    ...
                    case 2:
                        system ("cls");
                        b.extratohtml(cont);
                        system(" C:\\Users\\Administrator\\Desktop\\arabanco\\extrato.html");
                        system("pause");
